...
if (!isset($_POST['nom']) || empty(trim($_POST['nom']))) {
    echo "<p style='color: red;'>Nom de salle manquant.</p>";
    exit;
}

$salle_nom = htmlspecialchars($_POST['nom']);

$stmt = $pdo->prepare("SELECT * FROM salles WHERE nom = ?");
$stmt->execute([$salle_nom]);
$salle = $stmt->fetch();

if (!$salle) {
    echo "<p style='color: red;'>Salle non trouvée.</p>";
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['date_reservation'], $_POST['heure_reservation'])) {
    ...
    try {
        $stmt = $pdo->prepare("INSERT INTO reservations (utilisateur_id, type_etablissement, etablissement_id, date_reservation, heure_reservation, statut, date_creation_res) 
                               VALUES (?, 'salle', ?, ?, ?, 'confirmée', NOW())");
        $stmt->execute([$utilisateur_id, $salle['id'], $date_reservation, $heure_reservation]);
...
<div class="container">
    <h1>Réserver la salle : <?= htmlspecialchars($salle['nom']) ?></h1>
