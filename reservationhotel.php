...
if (!isset($_POST['nom']) || empty(trim($_POST['nom']))) {
    echo "<p style='color: red;'>Nom d'hôtel manquant.</p>";
    header('Location: Login.php');
    exit;
}

$hotel_nom = htmlspecialchars($_POST['nom']);

$stmt = $pdo->prepare("SELECT * FROM hotels WHERE nom = ?");
$stmt->execute([$hotel_nom]);
$hotel = $stmt->fetch();

if (!$hotel) {
    echo "<p style='color: red;'>Hôtel non trouvé.</p>";
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['date_reservation'], $_POST['heure_reservation'])) {
    ...
    try {
        $stmt = $pdo->prepare("INSERT INTO reservations (utilisateur_id, type_etablissement, etablissement_id, date_reservation, heure_reservation, statut, date_creation_res) 
                               VALUES (?, 'hotel', ?, ?, ?, 'confirmée', NOW())");
        $stmt->execute([$utilisateur_id, $hotel['id'], $date_reservation, $heure_reservation]);

        // Redirection vers le menu après succès
...
<div class="container">
    <h1>Réserver l'Hôtel : <?= htmlspecialchars($hotel['nom']) ?></h1>
    <p class="description"><?= htmlspecialchars($hotel['description']) ?></p>

    <form method="POST">
        <input type="hidden" name="nom" value="<?= htmlspecialchars($hotel['nom']) ?>">
